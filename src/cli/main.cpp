...
static void usage(const char* prog) {
    std::fprintf(stderr,
        ...
        "             Theory-First (Paper D v0.3): Frobenius / Sato-Tate quantization\n"
        "\n"
        "Usage: %s <command> [options]\n"
        "\n"
        "Primary commands:\n"
        "  run        --model <gguf> [opts] <prompt>     One-shot generate.\n"
        "  chat       --model <gguf> [opts] <prompt>     Interactive chat.\n"
        "  perplexity --model <gguf> [opts] <textfile>   Bench PPL on a corpus.\n"
        "  server     --model <gguf> [opts]              HTTP OpenAI-compat (Phase 2).\n"
        "\n"
        "Inspection:\n"
        "  version                          Print version + commit.\n"
        "  banner                           Print banner + loaded submodule SHAs.\n"
        "  info       --model <gguf>        Load a GGUF and print hparams + tensors.\n"
        "  encode     --model <gguf> <text> Tokenize text → IDs.\n"
        "  decode     --model <gguf> <ids>  Decode IDs → text.\n"
        "  embed      --model <gguf> <text> Token embedding lookup.\n"
        "\n"
        "─────────────────────────────────────────────────────────────────────────\n"
        ...
        "                               split prime p2 (bounded-drift channel).\n"
        "                               Default: 2,2,41,8 (≈ fp10 total bit-width).\n"
        "\n"
        "─────────────────────────────────────────────────────────────────────────\n"
        "Standard options\n"
        "─────────────────────────────────────────────────────────────────────────\n"
        "  --model <path.gguf>          Model file (GGUF format).\n"
        "  --ctx <n>                    Context length. Default 2048.\n"
        "  --n-gpu-layers <n>           Layers to offload (req: SP_ENGINE_BACKEND).\n"
        "  --n-gpus <n>                 Multi-GPU sharding (0=auto, 1=single).\n"
        "  --model-preset <n>           Arch overlay: auto | off | <name>.\n"
        "\n"
        "Hot/cold tiered storage (GPU↔CPU offload):\n"
        "  --cold                       Enable cold storage (GPU→CPU mirror).\n"
        "  --cold-mb <n>                Cold storage with N-MB ring buffer.\n"
        "  --evict-keep <n>             Keep N recent positions on GPU.\n"
        "\n"
        "Disk serialisation:\n"
        ...
        "\n"
        "─────────────────────────────────────────────────────────────────────────\n"
        "DEPRECATED OPTIONS — slated for removal in Stage 3 cleanup.\n"
        "Subsumed by --frobenius-quant / --sato-tate-mix above.\n"
        "See docs/CLEANUP_PLAN.md for migration paths.\n"
        "─────────────────────────────────────────────────────────────────────────\n"
        "  --sqfree, --spinor, --no-mobius\n"
        "  --hierarchical, --no-compression, --hier-level, --hier-res-bits,\n"
        "  --hier-res-bits-v, --hier-skel-bits, --hier-ternary-mask\n"
        "  --k-bits, --v-bits, --residual-bits\n"
        "  --ternary-k, --ternary-v, --fp8\n"
        "  --pe-mode, --pe-alpha, --pe-tier\n"
        "  --cauchy-mode, --cauchy-fixed-n, --cauchy-cooldown, --cauchy-warmup,\n"
        "  --cauchy-use-ricci, --cauchy-ricci-only, --cauchy-mertens-only, --params-b\n"
        "  --system12, --s12-threshold, --s12-sys2\n"
        "  --crt-split, --moe-curriculum, --beast\n"
        "\n"
        "─────────────────────────────────────────────────────────────────────────\n"
        "DEPRECATED VERBS — slated for removal in Stage 2 cleanup.\n"
        "─────────────────────────────────────────────────────────────────────────\n"
        "  kv_smoke, block1, logits, prefill, cache_ppl\n"
        "  (cache_ppl will fold into `perplexity --correlation` in Stage 2.)\n"
        "\n"
        "─────────────────────────────────────────────────────────────────────────\n"
        "Environment variables\n"
        "─────────────────────────────────────────────────────────────────────────\n"
        "  SP_ENGINE_BACKEND        cpu | cuda | vulkan\n"
        "  SP_ENGINE_THEORY_FIRST   1 (default) — banner indicates Theory-First path\n"
        "\n", prog);
}
...
static int parse_config_flag(sp::engine::Config& cfg, const char* a, const char* next) {
    ...
    // -- Phase 0 Theory-First: Frobenius / Sato-Tate quant (Paper D v0.3) --
    ...
    if (a_eq("--sato-tate-mix") && has_next) {
        ...
        long long a, b, c, d;
        if (std::sscanf(next, "%lld,%lld,%lld,%lld", &a, &b, &c, &d) == 4) {
            cfg.sato_tate_mix = true;
            cfg.st_p1 = (int64_t)a; cfg.st_k1 = (int64_t)b;
            cfg.st_p2 = (int64_t)c; cfg.st_k2 = (int64_t)d;
        }
        return 2;
    }
...
int main(int argc, char** argv) {
    ...
    if (cmd == "perplexity") {
        ...
        if (native) {
            ...
            // Phase 9b (post Plan C): dual-prime CRT is the only
            // NTT path. Default ON whenever attn_mode == 1 (poly-ring
            // active). Explicit SP_ENGINE_POLY_NTT_CRT=0 falls back to
            // scalar O(N^2) sp_poly_dot_product (correctness preserved,
            // speed lost).
            ctx.k_ntt_crt = (ctx.attn_mode == 1);
