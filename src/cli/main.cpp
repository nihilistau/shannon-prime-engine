...
int main(int argc, char** argv) {
    ...
    if (cmd == "perplexity") {
        ...
        if (native) {
            ...
            // Phase 7: persistent NTT-domain K cache.
            ...
            if (ctx.attn_mode == 1 && ctx.k_ntt_persist
                ...
                ctx.k_ntt_cache.assign(slab_count, 0);
                std::fprintf(stderr,
                    "[sp-engine] perplexity-native: K_NTT persistent cache = "
                    "%.1f MB (n_layers=%d, n_kv_head=%d, n_ctx=%d, N=%d)\n",
