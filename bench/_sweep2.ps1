...
foreach ($tau in @(0.05, 0.10, 0.20, 0.40)) {
    foreach ($alpha in @(0.3, 0.5, 0.7)) {
        # kill any lingering engine before launching a new one
        ...
        $args = @('perplexity','--model',$model,'--ctx','64','--chunks','1','--frobenius-quant','--friedman-sieve','--friedman-mode','policy','--kste-tau-A',("{0:F4}" -f $tau),'--kste-alpha',("{0:F4}" -f $alpha),'test_corpus.txt')
        ...
        if (-not (Wait-Process -Id $p.Id -Timeout 240 -ErrorAction SilentlyContinue -PassThru)) {
        ...
        $line = ("tau={0:F3} alpha={1:F2} PPL={2:F4} delta={3:+0.00;-0.00}% evict={4:F2}%" -f $tau, $alpha, $ppl, $delta, $evict)
